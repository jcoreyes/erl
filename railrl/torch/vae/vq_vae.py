...
#             self._vq_vae = VectorQuantizer(num_embeddings, self.embedding_dim,
#                                            commitment_cost)
#         self._vq_decoder = Decoder(self.embedding_dim,
#                                 num_hiddens,
#                                 num_residual_layers,
#                                 num_residual_hiddens)
#         self._vae_decoder = Decoder(self.embedding_dim,
#                                 num_hiddens,
#                                 num_residual_layers,
#                                 num_residual_hiddens)
...
#         vq_loss, quantized, perplexity, _ = self.quantize_image(inputs)
#         vq_recon = self.decode(quantized, quantized=True)

#         z_s, kle = self.bottleneck_image(inputs)
#         vae_recon = self.decode(z_s, quantized=False)
        
#         vq_recon_error = F.mse_loss(vq_recon, inputs, reduction='sum')
#         vae_recon_error = F.mse_loss(vae_recon, inputs, reduction='sum')
#         return vq_loss, kle, perplexity, vq_recon, vae_recon, vq_recon_error, vae_recon_error
...
#     def bottleneck_image(self, inputs):
#         inputs = inputs.view(-1,
#                             self.input_channels,
#                             self.imsize,
#                             self.imsize)
...
#         return ptu.randn(batch_size, self.representation_size)
...
#     def decode(self, latents, quantized=True):
#         latents = latents.reshape(-1, self.embedding_dim, self.root_len, self.root_len)
#         if quantized:
#             return self._vq_decoder(latents)
