...
#     def compress(self, conv_latent):
#         conv_latent = conv_latent.view(-1, self.conv_size)
#         z = self.f_enc(conv_latent).view(-1, self.embedding_dim, self.root_len, self.root_len)
#         return z

#     def decompress(self, z):
#         z = z.view(-1, self.representation_size)
#         conv_latent = self.f_dec(z).view(-1, self.embedding_dim, self.root_conv_size, self.root_conv_size)
#         return conv_latent
...
class CVQVAE2(nn.Module):
    def __init__(
        ...
        self.rep_size = 16
        self.mu = nn.Linear(num_hiddens * 2 + self.latent_sizes[1], self.rep_size)
        self.logvar = nn.Linear(num_hiddens * 2 +  self.latent_sizes[1], self.rep_size)
        self.f = nn.Linear(self.rep_size, self.discrete_size)

        #self.mu = nn.Linear(self.discrete_size, self.discrete_size)
        #self.logvar = nn.Linear(self.discrete_size, self.discrete_size)

        #self.logvar = nn.Parameter(ptu.randn(self.discrete_size))


        self.mu.weight.data.uniform_(-1e-3, 1e-3)
        self.mu.bias.data.uniform_(-1e-3, 1e-3)
        self.logvar.weight.data.uniform_(-1e-3, 1e-3)
        self.logvar.bias.data.uniform_(-1e-3, 1e-3)
        self.f.weight.data.uniform_(-1e-3, 1e-3)
        self.f.bias.data.uniform_(-1e-3, 1e-3)
        ...
        if decay > 0.0:
            ...
            self._vq_vae = VectorQuantizer(num_embeddings, 1,
                                           commitment_cost, gaussion_prior=False)
    ...
    def compute_loss(self, obs, cond):
        ...
        z_mu, log_var = self.spatial_encoder(z, z_cond)
        z_t = self.reparameterize(z_mu, log_var)
        kle = self.kl_divergence(z_mu, log_var)
        z_big = self.f(z_t).reshape(-1, 1, 12, 12)

        vq_loss, quantized, perplexity, _ = self._vq_vae(z_big.detach())
        cond_vq_loss, cond_quantized, cond_perplexity, _ = self.cond_vq_vae(z_cond)
        
        cat_quantized = torch.cat([z_big, cond_quantized], dim=1)
    ...
    def encode(self, obs, cond, cont=True):
        ...
        _, quantized, _, encodings = self._vq_vae(z_big)
        _, cond_quantized, _, cond_encodings = self.cond_vq_vae(z_cond)

        if cont:
            z, z_c = z_t, cond_quantized
            #z, z_c = quantized, cond_quantized
        else:
            z, z_c = encodings, cond_encodings
    ...
    def reparameterize(self, quantized, logvar):
        if self.training:
            latent = self.rsample(quantized, logvar)
        else:
            latent = quantized
        return latent
    ...
    def spatial_encoder(self, latent, z_cond):
        ...
        # total = latent.sum(2).sum(2).view(latent.shape[0], latent.shape[1], 1, 1)
        # latent = latent / total
        ...
        #mu = latent.reshape(-1, self.discrete_size)
        #logvar = self.log_min_variance + torch.abs(self.logvar)
        mu = self.mu(latent)
    ...
    def decode(self, latents, cont=True):
        if cont:
            z_t = latents[:, :self.rep_size]
            z_big = self.f(z_t)
            z_0 = latents[:, self.rep_size:]
            latents = torch.cat([z_big, z_0], dim=1)
            z = latents.reshape(-1, self.embedding_dim + 1, self.root_len, self.root_len)
            #z = latents.reshape(-1, 2 * self.embedding_dim, self.root_len, self.root_len)
        else:
            z = self.conditioned_discrete_to_cont(latents)
    ...
    def sample_prior(self, batch_size, cond, cont=True):
        ...
        z = ptu.randn(batch_size, self.rep_size)
        z_big = self.f(z).reshape(-1, 1, 12, 12)
        _, quantized, _, encodings = self._vq_vae(z_big)
