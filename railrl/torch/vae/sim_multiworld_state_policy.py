...
def rollout(env, agent, max_path_length=np.inf, animated=False, obs_key='state_observation', goal_key='state_desired_goal'):
    """
    ...
    while path_length < max_path_length:
        obs = np.hstack((o[obs_key], goal[goal_key]))
...
def sim(env, policy, filename, horizon=100, animated=True):
    ...
    while True:
        ...
        for _ in range(1):
            ...
            paths.append(path)
        env_info = env.get_diagnostics(paths)
