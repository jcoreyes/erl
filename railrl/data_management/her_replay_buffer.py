...
class AutoHerReplayBuffer(EnvReplayBuffer):
    """
    Assumes a MultitaskToFlatEnv for relabeling purposes.
    """
    def __init__(
            self,
            max_size,
            env,
            p_relabel = 0, # only relabel this many goals, otherwise keep original
            p_her = 0, # when relabeling, this percent is HER
            her_replay_strategy="future", # for HER goals, what strategy
            num_goals_to_sample=4, # for future HER, how many goals
    ):
        super().__init__(max_size, env)
        self.num_goals_to_sample = num_goals_to_sample
        self._goals = np.zeros((max_size, self.env.goal_dim))
    ...
    def random_batch(self, batch_size):
        # sample new goals according to params and recompute rewards
        indices = np.random.randint(0, self._size, batch_size)
        return dict(
            observations=self._observations[indices],
            actions=self._actions[indices],
