...
if __name__ == '__main__':
    ...
    DATA_DIR = '/media/ashvin/bair/softmotion30_44k/train'
    # '/'.join(str.split(current_dir, '/')[:-3]) + '/pushing_data/softmotion30/test'
    # conf['canon_ex'] = ""
    ...
    conf['sequence_length']= H      # 'sequence length, including context frames.'
    conf['use_state'] = True
    conf['batch_size']= B
    conf['visualize']= True
    # conf['single_view'] = ''
    ...
    total_trajectories = 40192 # test: 256
    trajectories_per_numpy_file = 1024
    for k in range(total_trajectories / trajectories_per_numpy_file):
        N = trajectories_per_numpy_file / B # number of trajectories
        K = 2 # number of cameras
        # B = loading batch size
        # H = horizon
        all_images = np.zeros((N, B, K, H, 64, 64, 3), dtype=np.uint8)
        all_actions = np.zeros((N, B, H, 4))
        all_endeff = np.zeros((N, B, H, 3))
        ...
        for i in range(N):
            ...
            if 'canon_ex' in conf:
                image_main, image_aux, actions, endeff, init_pix_distrib, init_pix_pos  = sess.run([image_aux_batch, action_batch, endeff_pos_batch, pix_distrib_batch, pix_pos_batch])
            else:
                image_main, image_aux, actions, endeff = sess.run([image_main_batch, image_aux_batch, action_batch, endeff_pos_batch])

            print image_main.shape, image_aux.shape, actions.shape, endeff.shape

            image_main = np.uint8(255. * image_main)
            image_aux = np.uint8(255. * image_aux)

            all_images[i, :, 0, :, :, :, :] = image_main
            all_images[i, :, 1, :, :, :, :] = image_aux
            ...
            #     image_main = np.squeeze(image_main)
            #     img = np.uint8(255. * image_main[i, 0])
            #     img = Image.fromarray(img, 'RGB')
            #     # img.save(file_path,'PNG')
            #     img.show()
            #     print i
            ...
            # print(np.all(actions == all_actions[0, :, :, :]))
        ...
        np.savez('/media/ashvin/bair/softmotion30_44k/numpy/train{:0>2d}.npz'.format(k),
            images = all_images,
            actions = all_actions,
            endeff = all_endeff,
        )