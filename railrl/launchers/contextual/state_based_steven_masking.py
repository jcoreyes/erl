...
class MaskedGoalDictDistributionFromMultitaskEnv(
    ...
    def sample(self, batch_size: int, use_env_goal=False):
        goals = super().sample(batch_size, use_env_goal=use_env_goal)
        if self.distribution_type == 'static_mask':
...
def masking_sac_experiment_with_variant(variant):
    max_path_length = variant['max_path_length']
    qf_kwargs = variant['qf_kwargs']
    sac_trainer_kwargs = variant['sac_trainer_kwargs']
    replay_buffer_kwargs = variant['replay_buffer_kwargs']
    ...
    exploration_policy_kwargs = variant.get('exploration_policy_kwargs', None)
    evaluation_goal_sampling_mode = variant.get('evaluation_goal_sampling_mode', None)
    exploration_goal_sampling_mode = variant.get('exploration_goal_sampling_mode', None)
    # Video parameters
    save_video = variant.get('save_video', True)
    save_video_kwargs = variant.get('save_video_kwargs', None)
    renderer_kwargs = variant.get('renderer_kwargs', None)
    train_env_id = variant.get('train_env_id', None)
    eval_env_id = variant.get('eval_env_id', None)

    do_masking = variant.get('do_masking', True)
    mask_key = variant.get('mask_key', "masked_observation")
    masking_eval_steps = variant.get('masking_eval_steps', 200)
    log_mask_diagnostics = variant.get('log_mask_diagnostics', True)
    mask_dim = variant.get('mask_dim', None)
    masking_reward_fn = variant.get('masking_reward_fn', None)
    masking_for_exploration = variant.get('masking_for_exploration', True)
    rotate_masks_for_eval = variant.get('rotate_masks_for_eval', False)
    rotate_masks_for_expl = variant.get('rotate_masks_for_expl', False)

    mask_distribution = variant.get('mask_distribution', None)
    num_steps_per_mask_change = variant.get('num_steps_per_mask_change', 10)

    tag = variant.get('tag', None)
    ...
    masking_sac_experiment(
        ...
        exploration_policy_kwargs=exploration_policy_kwargs,
        evaluation_goal_sampling_mode=evaluation_goal_sampling_mode,
        exploration_goal_sampling_mode=exploration_goal_sampling_mode,
        # Video parameters
        ...
        mask_key=mask_key,
        masking_eval_steps=masking_eval_steps,
        log_mask_diagnostics=log_mask_diagnostics,
        mask_dim=mask_dim,
        masking_reward_fn=masking_reward_fn,
        masking_for_exploration=masking_for_exploration,
        rotate_masks_for_eval=rotate_masks_for_eval,
        rotate_masks_for_expl=rotate_masks_for_expl,
