...
def her_td3_experiment(variant):
    if variant.get('presample_goals', None):
        presampled_goals = variant['presample_goals']()
        env = variant['env_class'](presampled_goals=presampled_goals, **variant['env_kwargs'])
    else:
    ...
    if variant['normalize']:
        env = NormalizedBoxEnv(env, obs_to_normalize_keys=variant.get('obs_to_normalize_keys', (observation_key)))
