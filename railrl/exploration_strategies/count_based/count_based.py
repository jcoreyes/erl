...
class CountExploration:
    def __init__(self,
        ...
        self.obs_dim = obs_dim if obs_dim != 0 else env.observation_space.spaces[observation_key].low.size
        self.hash_dim = hash_dim
        if hashing_matrix is not None:
            self.hashing_matrix = hashing_matrix
        else:
            self.hashing_matrix = np.reshape(np.random.normal(0, 1, self.obs_dim * self.hash_dim), (self.obs_dim, self.hash_dim))
        self.counts = Counter()
