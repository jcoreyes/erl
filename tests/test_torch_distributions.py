...
class TestNormalDistribution(NPTestCase):
    ...
    def test_log_prob_type(self):
        normal = Normal(0, 1)
        x = ptu.np_to_var(np.array([0]))
        log_prob = normal.log_prob(x)
        self.assertIsInstance(log_prob, ptu.TorchVariable)
    ...
    def test_log_prob_gradient(self):
        ...
        :return:
        """
        mean_var = ptu.np_to_var(np.array([0]), requires_grad=True)
        std_var = ptu.np_to_var(np.array([0.25]), requires_grad=True)
        normal = Normal(mean_var, std_var)
        x = ptu.np_to_var(np.array([1]))
        log_prob = normal.log_prob(x)

        gradient = ptu.np_to_var(np.array([1]))
