...
f = plot.filter_by_flat_params({'replay_kwargs.fraction_goals_are_env_goals': 0.5, 'algo_kwargs.reward_scale': 1e-4})
...
dsae = plot.load_exps([ashvin_base_dir + 's3doodad/share/steven/pushing-multipushing/multipusher-reward-variants-spatial'], f, suppress_output=True)
...
her = plot.load_exps([ashvin_base_dir + 's3doodad/share/steven/multipush-her-images'], f, suppress_output=True)
plot.tag_exps(her, "name", "her")
her2 = plot.load_exps([ashvin_base_dir + 's3doodad/share/steven/multipush-her-images'], f, suppress_output=True)
plot.tag_exps(her2, "name", "l&r")

plot.comparison(ours + oracle + dsae + her + her2, ["Final  total_distance Mean"], 
            vary = ["name"],
           default_vary={"replay_strategy": "future"},
          smooth=plot.padded_ma_filter(10),
          xlim=(0, 250000), ylim=(0.15, 0.3), figsize=(7,3.5),
...
plt.legend(["GRiLL", "DSAE", "HER", "Oracle", "L&R", ], bbox_to_anchor=(1.0, 0.5), loc="center left")
plt.tight_layout()
plt.savefig(output_dir + "multiobj_push_baselines.pdf")
print("File saved to", output_dir + "multiobj_push_baselines.pdf")
